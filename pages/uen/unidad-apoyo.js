...
const UnidadDeApoyo = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const savedPresupuesto = await getDataFromDB("selectedPresupuesto", "unidad-apoyo_selectedPresupuesto");
        ...
        if (savedPresupuesto?.updatedRubros?.length > 0) {
          ...
          setInputValues(savedPresupuesto.inputs || {});
          setCentroCostoid(savedPresupuesto.centroCostoid || []);
        } else if (savedData?.updatedRubros?.length > 0) {
